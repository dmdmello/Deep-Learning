...
if int(sys.argv[3]) != 0:
	...
	time_file = 'Transp-%s-shape-%s-%s' % (device_name,shape[1], shape[2])
else: 
	shape = (int(sys.argv[2]), int(sys.argv[2]))
	time_file = 'Transp-%s-shape-%s-%s' % (device_name,shape[0], shape[1])
if device_name == "gpu":
	device_name = "/gpu:0"
else:
	device_name = "/cpu:0"
...
with tf.device(device_name):
	...
	transp_operation = tf.transpose(random_matrix)

t1 = time.time()
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as session:
	result = session.run(transp_operation)
t2 = time.time()
time = (t2 - t1) * 1000.
