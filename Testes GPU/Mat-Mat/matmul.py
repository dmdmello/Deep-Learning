...
if int(sys.argv[3]) != 0:
	...
	time_file = 'MatMul-%s-shape-%s-%s' % (device_name,shape[1], shape[2])
else: 
	shape = (int(sys.argv[2]), int(sys.argv[2]))
	time_file = 'MatMul-%s-shape-%s-%s' % (device_name,shape[0], shape[1])
if device_name == "gpu":
	device_name = "/gpu:0"
else:
	device_name = "/cpu:0"
...
with tf.device(device_name):
	...
	dot_operation = tf.matmul(random_matrix, random_matrix)
